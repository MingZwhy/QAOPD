...
export PROJECT_NAME=opd_qad_w279_math_search
...
case "$VARIANT" in
    ...
    fkl64_teacher17_steps20)
        ...
        export SAVE_FREQ=5
        ;;
    rkl_overlap64_teacher17_steps20)
        export TEACHER_MODEL=$ROOT/models/Qwen3-1.7B
        export DISTILLATION_LOSS_MODE=reverse_kl_topk_overlap
        export USE_POLICY_GRADIENT=False
        export DISTILLATION_TOPK=64
        export DISTILLATION_LOSS_COEF=1.0
        export ACTOR_LR=1e-6
    ...
    *)
        ...
        echo "  k1_teacher17_lr1e6_steps30" >&2
        echo "  k1_teacher8_steps20" >&2
        echo "  fkl64_teacher17_steps20" >&2
        echo "  rkl_overlap64_teacher17_steps20" >&2
        exit 2
        ;;
esac

export EXPERIMENT_NAME=${EXPERIMENT_NAME:-w279a8kv16_math_${VARIANT}_${RUN_STAMP}}
export OUTPUT_DIR=${OUTPUT_DIR:-$ROOT/experiments/opd_qad/math_search/$EXPERIMENT_NAME}

echo "Math OPD search configuration"
...
cat > "$OUTPUT_DIR/search_manifest.json" <<EOF
{
  ...
  "student_model": "${MODELS_DIR:-$QAOPD_ROOT/models}/w279_step10000",
  ...
  "evaluation_target": "W2.79A8KV16 GSM8K 5-shot batch16"
}
EOF
